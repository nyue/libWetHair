...
public:
  ...
  void accumulateExternalGradU( VectorXs& F, const VectorXs& dx = VectorXs(), const VectorXs& dv = VectorXs() );
  
  void accumulateGradU( VectorXs& F, const VectorXs& dx = VectorXs(), const VectorXs& dv = VectorXs() );

  void accumulateExternalddUdxdx( TripletXs& A, const VectorXs& dx = VectorXs(), const VectorXs& dv = VectorXs() );

  // Kind of a misnomer.
  void accumulateExternalddUdxdv( TripletXs& A, const VectorXs& dx = VectorXs(), const VectorXs& dv = VectorXs() );
  ...
  scalar getStiffnessPlanar(const scalar& radius, const scalar& d0, const scalar& A_target, const scalar& pressure_weight) const;
  
  scalar getDStar(const scalar& radius) const;
  
  scalar getStiffness(const scalar& radius, const scalar& d0, const scalar& A_target, const scalar& pressure_weight) const;
  ...
  void preComputeLocal(const VectorXs& dx, const VectorXs& dv, const scalar& dt);
  
  void preComputeInterhair(const VectorXs& dx, const VectorXs& dv, const scalar& dt);
  ...
  void localPostPreprocess(VectorXs& lambda, VectorXs& lambda_v,
                      TripletXs& J, TripletXs& Jv, TripletXs& Jxv, TripletXs& tildeK,
                      TripletXs& stiffness, TripletXs& damping, VectorXs& Phi, VectorXs& Phiv, const VectorXs& dx, const VectorXs& dv, const scalar& dt);
  ...
  void updateNumConstraints(int& num_constraint_pos, int& num_constraint_vel, int& num_J, int& num_Jv, int& num_Jxv, int& num_tildeK,
                            Vector6i& interhair_param, Vector6i& interhair_num);
  
  void updateNumConstraintsLocal(int& num_constraint_pos, int& num_constraint_vel, int& num_J, int& num_Jv, int& num_Jxv, int& num_tildeK);
  
  void updateNumConstraintsInterHair(int& num_constraint_pos, int& num_constraint_vel, int& num_J, int& num_Jv, int& num_Jxv, int& num_tildeK, Vector6i& interhair_param, Vector6i& interhair_num);
  ...
  void absorbReleaseParticleFlows(const scalar& dt, bool noabsorb, bool nodripping);
  ...
  void updateMassWithLiquid(int idx, const scalar& liquid_mass, const scalar& liquid_radii);
  
  void saveFluidOff( const std::string& szfn );
  ...
  const TwoDSceneSerializer<DIM>& getSerializer() const;
  TwoDSceneSerializer<DIM>& getSerializer();

  void setVertToDoFMap( const std::vector<int>& vert_to_dof, const VectorXi& dofs_to_vars, const std::vector<bool>& tipVerts, const VectorXi& dof_to_vert );

  bool confirmNotNeighborEdgesOnSameHair( const int& efirst, const int& esecond ) const;
