...
void StrandForce::addEnergyToTotal( const VecX& x, const VecX& v, const VecX& m, scalar& E )
{
    m_strandEnergyUpdate = 0.;

    const VecX currStrandDoFs = x.segment( m_scene->getDof( m_verts[0] ), getNumVertices() * 4 - 1 );
    if( currStrandDoFs != m_strandState->m_dofs.get() ){
        m_strandState->m_dofs.set( currStrandDoFs );
        // not updating startState since we are not computing viscous energy
    }

    ForceAccumulator< StretchingForce< NonViscous > >::accumulate( m_strandEnergyUpdate, *this );
    ForceAccumulator< TwistingForce< NonViscous > >::accumulate( m_strandEnergyUpdate, *this );
    ForceAccumulator< BendingForce< NonViscous > >::accumulate( m_strandEnergyUpdate, *this );

    // if( m_strandParams->m_accumulateWithViscous ){
    //     std::cerr << outputmod::startred << "WARNING IN StrandForce: " << outputmod::endred 
    //     << "energy defined for StrandForce is dissipative. Viscous component enabled..." << std::endl;    
    // }
    E += m_strandEnergyUpdate;
}

void StrandForce::addGradEToTotal( const VecX& x, const VecX& v, const VecX& m, VecX& gradE )
{
    std::cerr << "StrandForce addGradEToTotal not currently implemented" << std::endl; exit(1);
    // updateDoFs( x, v, dt );
    // // Map accumulated strandForceUpdate back to gradE, assuming the verts are consecutive in m_scene->getX()
    // gradE.segment( m_scene->getDof( m_verts[0] ), getNumVertices() * 4 - 1 ) += ( FORCE_SIGN ) * m_strandForceUpdate;
}

void StrandForce::addHessXToTotal( const VecX& x, const VecX& v, const VecX& m, TripletXs& hessE )
{
    std::cerr << "StrandForce addHessXToTotal not currently implemented" << std::endl; exit(1);
/*
    updateDoFs( x );

    // Map accumulated strandHessianUpdate back to hessE using m_verts:
    for( int t = 0; t < m_strandHessianUpdate.size(); ++t ){
        int i = m_strandHessianUpdate[t].row() / 4;
        int j = m_strandHessianUpdate[t].col() / 4;
        scalar value = ( HESS_SIGN ) * m_strandHessianUpdate[t].value();

        int iOffset = m_strandHessianUpdate[t].row() % 4;
        int jOffset = m_strandHessianUpdate[t].col() % 4;

        int idx = m_verts[i];
        int jdx = m_verts[j];        
        hessE.push_back( Triplets( m_scene->getDof( idx ) + iOffset, m_scene->getDof( jdx ) + jOffset, value ) );
    }
*/
}

void StrandForce::addHessVToTotal( const VecX& x, const VecX& v, const VecX& m, TripletXs& hessE )
{
    /* nothing to do, HessV handled via viscous X */
}

void StrandForce::addGradEToTotal( const VecX& x, const VecX& v, const VecX& m, VecX& gradE, int pidx )
{
    std::cerr << "StrandForce addGradEToTotal pidx not currently implemented" << std::endl; exit(1);
}

void StrandForce::addHessXToTotal( const VecX& x, const VecX& v, const VecX& m, VecX& hessE, int pidx )
{ // this row update will include 11 dofs: for the 4 before, 4 current, and 3 next vertex involved at a V, unless root/tip
    std::cerr << "StrandForce addHessXToTotal pidx not currently implemented" << std::endl; exit(1);
    ...
    //         updateDoFsStencil(x, pidx);

    //         if( m_scene->isTip(ip) ){
    //             for (SparseRXs::InnerIterator it( m_hess, 4 * v + idir ); it; ++it)
    //             {
    //                 if( it.col() >= 4 * (v - 1) ){
    //                     hessE( m_scene->getDof( m_verts[0] ) + it.col() ) += (HESS_SIGN) * it.value();
    //                 }
    //             }
    //         }
    //         else if( v == 0 ){
    ...
    //             for (SparseRXs::InnerIterator it( m_hess, 4 * v + idir ); it; ++it)
    //             {
    //                 if( it.col() >= 4 * (v - 1) && it.col() < 4 * (v + 1) + 3 ){
    //                     hessE( m_scene->getDof( m_verts[0] ) + it.col() ) += (HESS_SIGN) * it.value();
    //                 }
    //             }                  
    //         }
...
void StrandForce::addHessVToTotal( const VecX& x, const VecX& v, const VecX& m, VecX& hessE, int pidx )
{
    /* nothing to do, HessV handled via viscous HessX */    
}
