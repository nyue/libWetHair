...
friction_multiplier_planar(0.35),
hairsteps(1),
swesteps(1),
...
template<int DIM>
void TwoDScene<DIM>::saveFluidOff( const std::string& szfn )
{
  if(!m_fluid_sim) return;
  
  m_fluid_sim->save_particles_off(szfn);
}
...
template<int DIM>
const TwoDSceneSerializer<DIM>& TwoDScene<DIM>::getSerializer() const
{
  return m_scene_serializer;
}

template<int DIM>
TwoDSceneSerializer<DIM>& TwoDScene<DIM>::getSerializer()
{
  return m_scene_serializer;
}
