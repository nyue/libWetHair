...
const static Vector3s liquid_color = Vector3s(64.0/255.0, 164.0/255.0, 223.0/255.0);
...
template<int DIM>
TwoDSceneRenderer<DIM>::TwoDSceneRenderer( const TwoDScene<DIM>& scene,
...
template<int DIM>
void TwoDSceneRenderer<DIM>::setDC(const TwoDimensionalDisplayController<DIM>* dc)
{
  m_dc = dc;
}
...
template<>
void TwoDSceneRenderer<3>::initializeBoundaryRenderer(const TwoDScene<3>& scene)
{
  ...
  for(int i = 0; i < nb; ++i)
  {
    ...
    if(b->type == FluidSim3D::BT_CAPSULE) {
      ...
      if(sb->parameter(3) > 0.0) {
        ...
        rcb.Create(4, Vector3s(sb->parameter(0), sb->parameter(1), sb->parameter(2)), sb->parameter(3));
      }
    }
  }
}
...
template<>
void TwoDSceneRenderer<2>::initializeBoundaryRenderer(const TwoDScene<2>& scene)
{
}
...
template<int DIM>
void TwoDSceneRenderer<DIM>::initializeCylinderRenderer( int num_points, const TwoDScene<DIM>& scene )
{
  double dtheta = 2.0*PI/((double)num_points);
  m_cylinder_points.resize(num_points);
  for(int i = 0; i < num_points; ++i)
  {
    m_cylinder_points[i] = Vector3s(0.0, cos((scalar)i * dtheta), sin((scalar)i * dtheta));
  }
...
template<int DIM>
void TwoDSceneRenderer<DIM>::initializeOpenGLRenderer(const TwoDScene<DIM>& scene)
{
  int np = scene.getNumParticles();
  ...
  glBufferDataARB( GL_ELEMENT_ARRAY_BUFFER, m_cylinder_elements.size() * sizeof(int), &m_cylinder_elements[0], GL_STATIC_DRAW);
  glBindBufferARB( GL_ELEMENT_ARRAY_BUFFER, 0 );
  
  glBindBufferARB( GL_ARRAY_BUFFER, m_vertex_hair_core );
  glBufferDataARB( GL_ARRAY_BUFFER, disc_arc * np * sizeof(Vector3f), NULL, GL_DYNAMIC_DRAW);
  
  glBindBufferARB( GL_ARRAY_BUFFER, m_vertex_hair_flow );
  glBufferDataARB( GL_ARRAY_BUFFER, disc_arc * np * sizeof(Vector3f), NULL, GL_DYNAMIC_DRAW);
  
  glBindBufferARB( GL_ARRAY_BUFFER, m_vertex_fluids );
  glBufferDataARB( GL_ARRAY_BUFFER, nfluids_particles * sizeof(Vector3f), NULL, GL_DYNAMIC_DRAW);
  
  glBindBufferARB( GL_ARRAY_BUFFER, 0 );
}

template<>
void TwoDSceneRenderer<2>::renderSolidCircle( const Vector2s& center, double radius ) const
{
...
template<>
void TwoDSceneRenderer<3>::renderSolidCircle( const Vector3s& center, double radius ) const
{
  glBegin(GL_TRIANGLE_FAN);
  glVertex3dv(center.data());
  
  for( std::vector<std::pair<double,double> >::size_type i = 0; i < m_circle_points.size(); ++i )
    glVertex3d(radius*m_circle_points[i].first+center.x(), radius*m_circle_points[i].second+center.y(), center.z());
  
  glVertex3d(radius*m_circle_points.front().first+center.x(), radius*m_circle_points.front().second+center.y(), center.z());
  glEnd();
}

template<int DIM>
void TwoDSceneRenderer<DIM>::writeTransformedHairFlow( std::ostream& o, const TwoDScene<DIM>& scene ) const
{
  // vertex
  ...
  {
    o << "f " << m_cylinder_elements[i * 4 + 0] << " " << m_cylinder_elements[i * 4 + 1] << " " << m_cylinder_elements[i * 4 + 2] << " " << m_cylinder_elements[i * 4 + 3] << std::endl;
  }
}


template<>
void TwoDSceneRenderer<3>::writeBoundaries(std::ostream& os_single, std::ostream& os_double) const
{
  auto& boundaries = ((const FluidSim3D*) m_scene->getFluidSim())->get_boundaries();
  
  int iCapsule = 0;
  int iRoundBox = 0;
  for(auto& b : boundaries)
  {
    if(b->type == FluidSim3D::BT_UNION || b->type == FluidSim3D::BT_INTERSECT) continue;
    FluidSim3D::SolidBoundary<3>* sb = (FluidSim3D::SolidBoundary<3>*) b;
    ...
    switch (sb->type) {
      case FluidSim3D::BT_CIRCLE:
        write_trimesh3d(sb->sign <= 0.0 ? os_double : os_single, m_icosphere.vertices, m_icosphere.indices, sb->center, Vector3s::Constant(sb->parameter(0) * inner_scale), sb->rot, sb->sign <= 0.0);
        break;
      case FluidSim3D::BT_BOX:
        if(sb->parameter(3) > 0.0) {
          write_trimesh3d(sb->sign <= 0.0 ? os_double : os_single, m_roundcornerboxes[iRoundBox].vertices, m_roundcornerboxes[iRoundBox].indices, sb->center, Vector3s::Constant(inner_scale), sb->rot, sb->sign <= 0.0);
        } else {
          write_box3d(sb->sign <= 0.0 ? os_double : os_single, sb->parameter.segment<3>(0) * 2.0 * inner_scale, sb->center, sb->rot, sb->sign <= 0.0);
        }
        ++iRoundBox;
        break;
      case FluidSim3D::BT_CAPSULE:
        write_trimesh3d(sb->sign <= 0.0 ? os_double : os_single, m_capsules[iCapsule].vertices, m_capsules[iCapsule].indices, sb->center, Vector3s::Constant(inner_scale), sb->rot, sb->sign <= 0.0);
        ++iCapsule;
        break;
      default:
        break;
...
template<>
void TwoDSceneRenderer<2>::writeBoundaries(std::ostream& os_single, std::ostream& os_double) const
{
...
template<int DIM>
void TwoDSceneRenderer<DIM>::updateOpenGLRenderer(const TwoDScene<DIM>& scene, bool updateDevice)
{
  ...
  if(DIM == 3) {
    for(int i = 0; i < np; ++i)
    {
      ...
      Eigen::Affine3d(Eigen::AngleAxisd(phi0, Vector3s(0, 0, 1)) * Eigen::AngleAxisd(gamma0, Vector3s(0, 1, 0))) *
      ...
      {
        // render hairs
        m_dynamic_hair_core[i * disc_arc + j] = toFloatVec((trh * m_cylinder_points[j]).eval());
        m_dynamic_hair_flow[i * disc_arc + j] = toFloatVec((trf * m_cylinder_points[j]).eval());
      }
    }
    
    const auto& fluid_particles = ((FluidSim3D*) scene.getFluidSim())->get_particles();
    ...
    for(int i = 0; i < nfp; ++i)
    {
      if(fluid_particles[i].type == PT_LIQUID) {
        m_dynamic_fluid_particles.push_back(Vector3f( fluid_particles[i].x(0), fluid_particles[i].x(1), fluid_particles[i].x(2) ));
      }
    }

	if (updateDevice && m_dynamic_fluid_particles.size() > 0) {
		glBindBufferARB(GL_ARRAY_BUFFER, m_vertex_fluids);
		glBufferSubDataARB(GL_ARRAY_BUFFER, 0, m_dynamic_fluid_particles.size() * sizeof(Vector3f), &m_dynamic_fluid_particles[0]);
	}
  }
  else {
	  for (int i = 0; i < np; ++i)
	  ...
	  const auto& fluid_particles = ((FluidSim2D*)scene.getFluidSim())->get_particles();
	  int nfp = fluid_particles.size();
	  m_dynamic_fluid_particles.resize(0);
	  m_dynamic_fluid_particles.reserve(nfp);
	  for (int i = 0; i < nfp; ++i)
	  {
		  if (fluid_particles[i].type == PT_LIQUID) {
			  m_dynamic_fluid_particles.push_back(Vector3f(fluid_particles[i].x(0), fluid_particles[i].x(1), 0.0));
		  }
	  }
  ...
  if(updateDevice) {
    glBindBufferARB(GL_ARRAY_BUFFER, m_vertex_hair_core);
    glBufferSubDataARB(GL_ARRAY_BUFFER, 0, m_dynamic_hair_core.size() * sizeof(Vector3f), &m_dynamic_hair_core[0]);
    
    glBindBufferARB(GL_ARRAY_BUFFER, m_vertex_hair_flow);
    glBufferSubDataARB(GL_ARRAY_BUFFER, 0, m_dynamic_hair_flow.size() * sizeof(Vector3f), &m_dynamic_hair_flow[0]);
...
template<>
void TwoDSceneRenderer<3>::renderParticleSimulation( const TwoDScene<3>& scene ) const
{
      ...
      for(auto& b : boundaries)
      {
        ...
        switch (sb->type) {
          case FluidSim3D::BT_CIRCLE:
            ...
            draw_trimesh3d(m_icosphere.vertices, m_icosphere.indices, sb->center,
                           Vector3s(sb->parameter(0) * inner_scale, sb->parameter(0) * inner_scale, sb->parameter(0) * inner_scale),
                           sb->rot);
            ...
            if(sb->sign > 0.0) {
              ...
              draw_trimesh3d(m_icosphere.vertices, m_icosphere.indices, sb->center, Vector3s(sb->parameter(0) * outer_scale, sb->parameter(0) * outer_scale, sb->parameter(0) * outer_scale), sb->rot);
          ...
          case FluidSim3D::BT_BOX:
            ...
            if(sb->parameter(3) > 0.0) {
              draw_trimesh3d(m_roundcornerboxes[iRoundBox].vertices, m_roundcornerboxes[iRoundBox].indices, sb->center,
                             Vector3s(inner_scale, inner_scale, inner_scale),
                             sb->rot);
            } else {
              draw_box3d(Vector3s(sb->parameter(0), sb->parameter(1), sb->parameter(2)) * 2.0 * inner_scale, sb->center, sb->rot);
            }
            glDisable(GL_POLYGON_STIPPLE);
            glEnable(GL_DEPTH_TEST);
            if(sb->sign > 0.0) {
              ...
              if(sb->parameter(3) > 0.0) {
                ...
                draw_box3d(Vector3s(sb->parameter(0), sb->parameter(1), sb->parameter(2)) * 2.0 * outer_scale, sb->center, sb->rot);
              }
              glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
            }
            if(sb->parameter(3) > 0.0) {
          ...
          case FluidSim3D::BT_CAPSULE:
            ...
            draw_trimesh3d(m_capsules[iCapsule].vertices, m_capsules[iCapsule].indices, sb->center,
                           Vector3s(inner_scale, inner_scale, inner_scale),
                           sb->rot);
            glDisable(GL_POLYGON_STIPPLE);
            glEnable(GL_DEPTH_TEST);
            if(sb->sign > 0.0) {
              glColor3d(0.85,0.85,0.85);
              glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
              draw_trimesh3d(m_capsules[iCapsule].vertices, m_capsules[iCapsule].indices, sb->center, Vector3s(outer_scale, outer_scale, outer_scale), sb->rot);
              glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
            }
            ++iCapsule;
            break;
...
template<>
void TwoDSceneRenderer<2>::renderParticleSimulation( const TwoDScene<2>& scene ) const
{
  assert( renderingutils::checkGLErrors() );
  ...
  if(sim) {
    ...
    if(m_draw_grid) {
      ...
      glLineWidth(1);
      draw_grid2d(sim->get_origin(), sim->cellsize(), sim->get_ni(), sim->get_nj());
      assert( renderingutils::checkGLErrors() );
    }
    ...
    if(m_draw_boundaries) {
      ...
      for(auto& b : boundaries)
      {
        if(b->type == FluidSim2D::BT_UNION || b->type == FluidSim2D::BT_INTERSECT) continue;
        ...
        switch (sb->type) {
          ...
          case FluidSim2D::BT_BOX:
            draw_box2d(sb->center - Vector2s(sb->parameter(0), sb->parameter(1)), sb->parameter(0) * 2., sb->parameter(1) * 2.);
            break;
          default:
            break;
        }
    ...
    if(m_draw_velocities) {
      ...
      for(int j = 0;j < sim->get_nj(); ++j) for(int i = 0; i < sim->get_ni(); ++i) {
        Vector2s pos = Vector2s((i+0.5)*sim->cellsize(),(j+0.5)*sim->cellsize()) + sim->get_origin();
        draw_arrow2d(pos, pos + 0.01 * sim->get_velocity(pos), 0.1 * sim->cellsize());
      }
      glDisable(GL_BLEND);
      assert( renderingutils::checkGLErrors() );
    }
...
template<int DIM>
void TwoDSceneRenderer<DIM>::renderVolumeGraph( const TwoDScene<DIM>& scene ) const
{
  ...
  for(int i = 0; i < window - 1; ++i)
  {
    ...
    scalar fvc = (hair_vol[start_idx + i] + free_vol[start_idx + i]) / (free_vol[0] + hair_vol[0]);
    scalar sy = (fvc + 0.25) * stretch_height;
    glVertex2d(sx, sy);
    
    scalar sx1 = (scalar) (i + 1) / (scalar) window * stretch_width;
    scalar sy1 = ((hair_vol[start_idx + i + 1] + free_vol[start_idx + i + 1]) / (free_vol[0] + hair_vol[0]) + 0.25) * stretch_height;
  ...
  {
    scalar sx = (scalar) i / (scalar) window * stretch_width;
    scalar sy = (hair_vol[start_idx + i] / (free_vol[0] + hair_vol[0]) + 0.25) * stretch_height;
    glVertex2d(sx, sy);
    
    scalar sx1 = (scalar) (i + 1) / (scalar) window * stretch_width;
    scalar sy1 = (hair_vol[start_idx + i + 1] / (free_vol[0] + hair_vol[0]) + 0.25) * stretch_height;
    glVertex2d(sx1, sy1);
  }
  glEnd();
...
template<int DIM>
int TwoDSceneRenderer<DIM>::renderDebuggingInfo( const HairFlow<DIM> &flow ) const
{
  ...
  if(m_pvm != PVM_NONE) {
    ...
    switch (m_pvm) {
      case PVM_ETA:
        //oss << "Flow " << flow.index() << " - vertex eta (Min: " << flow.getMinEta() << ", Max: " << flow.getMaxEta() << ", Pool: " << flow.getPoolSize() << ")";
        oss << "Flow " << flow.index() << " - vertex eta (Min: " << flow.getMinEta() << ", Max: " << flow.getMaxEta() << ")";
        break;
  ...
  if(m_evm != EVM_NONE) {
    ...
    switch (m_evm) {
      case EVM_AREA:
        oss << "Flow " << flow.index() << " - edge area (Min: " << flow.getMinAreaE() << ", Max: " << flow.getMaxAreaE() << ")";
        break;
      default:
        break;
    }
...
template<int DIM>
std::vector<renderingutils::Color>& TwoDSceneRenderer<DIM>::getParticleColors()
{
  return m_particle_colors;
}

template<int DIM>
const std::vector<renderingutils::Color>& TwoDSceneRenderer<DIM>::getParticleColors() const
{
  return m_particle_colors;
}
