...
template<int DIM>
void TwoDSceneSerializer<DIM>::serializeFluidReadable( TwoDScene<DIM>& scene, std::vector< std::ostringstream >& outputstreams ) const
{
...
template<int DIM>
void TwoDSceneSerializer<DIM>::serializeBoundariesReadable( const TwoDSceneRenderer<DIM>* renderer, TwoDScene<DIM>& scene, std::ostream& os_boundary_single, std::ostream& os_boundary_double)
{
  renderer->writeBoundaries(os_boundary_single, os_boundary_double);
}

template<int DIM>
void TwoDSceneSerializer<DIM>::serializeHairReadable( TwoDScene<DIM>& scene, std::ostream& outputstream ) const
{
...
template<int DIM>
void TwoDSceneSerializer<DIM>::serializePolygonalCohesionReadable( const TwoDSceneRenderer<DIM>* renderer, TwoDScene<DIM>& scene, std::ostream& os_pe, std::ostream& os_poe, std::ostream& os_ppp ) const
{
...
template<int DIM>
void TwoDSceneSerializer<DIM>::serializeShallowFlowReadable( const TwoDSceneRenderer<DIM>* renderer, TwoDScene<DIM>& scene, std::ostream& outputstream ) const
{
...
template<int DIM>
bool TwoDSceneSerializer<DIM>::deSerializeFluidReadable( TwoDScene<DIM>& scene, const std::vector< std::string > &filename_fluids )
{
  ...
  // sim->readReadableBoundary(os_boundary_single, os_boundary_double); // for now
  return f;
}

template<int DIM>
bool TwoDSceneSerializer<DIM>::deSerializeHairReadable( TwoDScene<DIM>& scene, const std::string &filename_hairs ) 
{
  bool h = false;
  ...
  if( file.is_open() )
  {
    ...
    for( HairFlow<DIM>* flow : flows) {
      ...
      flow->updateGeometricState( scene.getX(), scene.getV(), scene.getFluidSim() );
    }
  }
  file.close();
  return h;
...
template<int DIM>
bool TwoDSceneSerializer<DIM>::deSerializeShallowFlowReadable( const TwoDSceneRenderer<DIM>* renderer, TwoDScene<DIM>& scene, const std::string &filename_flows ) 
{
...
template<int DIM>
void TwoDSceneSerializer<DIM>::serializeScene( TwoDScene<DIM>& scene, SceneStepper<DIM>* stepper, std::ostream& outputstream ) const
{
...
template<int DIM>
void TwoDSceneSerializer<DIM>::loadScene( TwoDScene<DIM>& scene, SceneStepper<DIM>* stepper, std::istream& inputstream ) const
{
  int ndof = scene.getNumDofs();
  scalar* xdata = scene.getX().data();
  ...
  std::vector<scalar> buf;
  size_t datasize = flowdatasize + fluiddatasize + boundarydatasize + fluidcrucialgridsize + integratorsize;
