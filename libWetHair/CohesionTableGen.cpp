...
CohesionTable::CohesionTable(const scalar& radius_multiplier, const scalar& collision_stiffness, const scalar& radius_multiplier_planar, const scalar& collision_stiffness_planar)
: m_sigma(72.0), m_theta(M_PI / 4), m_radii(0.004), m_max_alpha(M_PI - m_theta), m_max_d0(0.2), m_min_d0(2.0 * m_radii), m_min_d0_planar(m_radii), m_radius_multiplier(radius_multiplier), m_collision_stiffness(collision_stiffness), m_collision_stiffness_planar(collision_stiffness_planar), m_radius_multiplier_planar(radius_multiplier_planar), m_discretization(256)
{}

void CohesionTable::print_energy_data(std::ostream& oss, bool first_time) const
{
...
void CohesionTable::print_table(std::ostream& oss, const MatrixXs& mat, const scalar& dmin) const
{
  scalar d_inc = m_max_d0 / (scalar) m_discretization;
...
scalar CohesionTable::interpolate_table(const scalar& A, const scalar& d0, const MatrixXs& mat, const scalar& dmin) const
{
  scalar d_inc = m_max_d0 / (scalar) m_discretization;
...
scalar CohesionTable::interpolate_table_grad(const scalar& A, const scalar& d0, const MatrixXs& mat, const scalar& dmin) const
{
  scalar d_inc = m_max_d0 / (scalar) m_discretization;
  scalar p = std::max(0., d0 - dmin) / d_inc;
  int ip0 = std::max(0, std::min(m_discretization - 2, (int) p));
...
{
  if(d0 < getDStarPlanar()) return computedEddAreaDistPlanar(A_target, getDStarPlanar());
...
void CohesionTable::construct_alpha_table()
{
  ...
  for(int i = 0; i < m_discretization; ++i)
  {
    ...
    for(int j = 0; j < m_discretization; ++j)
    {
      ...
      if(A1 == A0) alpha_target = retidx * alpha_inc;
      else alpha_target = ((A_target - A0) / (A1 - A0) * (scalar)(otheridx - retidx) + retidx) * alpha_inc;
...
scalar CohesionTable::getStiffness(const scalar& d0, const scalar& A_target, const scalar& pressure_weight) const
{
  ...
  if(d0 < d_hat) {
    if(d0 < dmin) {
      ...
      scalar dEdd_d_hat = A_target < dry_criterion ? 0.0 : (interpolate_dEdd(A_target, d_hat) * pressure_weight);
      
      scalar stiffness_d_hat = dEdd_d_hat / (d_hat - d_star);
      
      if(d0 < d_star) {
        // linear interpolate
        scalar stiffness = ((stiffness_d_hat - m_collision_stiffness) * d0 + m_collision_stiffness * d_star - stiffness_d_hat * dmin) / (d_star - dmin);
        return stiffness;
      } else {
    ...
    scalar dEdd_d0 = A_target < dry_criterion ? 0.0 : (interpolate_dEdd(A_target, d0) * pressure_weight);
    scalar stiffness_d0 = dEdd_d0 / (d0 - d_star);
    return stiffness_d0;
  }
}

scalar CohesionTable::getStiffnessPlanar(const scalar& d0, const scalar& A_target, const scalar& pressure_weight) const
{
  ...
  if(d0 < d_hat) {
    if(d0 < dmin) {
      ...
      scalar dEdd_d_hat = A_target < dry_criterion ? 0.0 : (interpolate_dEdd_planar(A_target, d_hat) * pressure_weight);
      
      scalar stiffness_d_hat = dEdd_d_hat / (d_hat - d_star);
      
      if(d0 < d_star) {
        // linear interpolate
        scalar stiffness = ((stiffness_d_hat - m_collision_stiffness_planar) * d0 + m_collision_stiffness_planar * d_star - stiffness_d_hat * dmin) / (d_star - dmin);
        return stiffness;
      } else {
        // extrapolate the stiffness at d_hat
        return stiffness_d_hat;
    ...
    scalar dEdd_d0 = A_target < dry_criterion ? 0.0 : (interpolate_dEdd_planar(A_target, d0) * pressure_weight);
    scalar stiffness_d0 = dEdd_d0 / (d0 - d_star);
    return stiffness_d0;
  }
}
