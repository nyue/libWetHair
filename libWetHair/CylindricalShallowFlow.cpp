...
HairFlow<DIM>(parent, involved_particles, eta, particle_state)
{
  ...
  compute_edge_val(HairFlow<DIM>::m_eta, HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_edge_eta); stringutils::print(HairFlow<DIM>::m_edge_eta);
...
{
  // compute geometry
  m_old_rad_vec = HairFlow<DIM>::m_rad_vec; stringutils::print(MAKE_REF(m_old_rad_vec));
  m_old_area_v = HairFlow<DIM>::m_area_v; stringutils::print(MAKE_REF(m_old_area_v));
  ...
  compute_radius_vector(HairFlow<DIM>::m_parent->getEdgeRadii(), HairFlow<DIM>::m_edge_indices, HairFlow<DIM>::m_edge_rad_vec); stringutils::print(MAKE_REF(HairFlow<DIM>::m_edge_rad_vec));
  
  if(HairFlow<DIM>::m_parent->isIndividualTransfer())
  {
    compute_edge_area<DIM>(x, HairFlow<DIM>::m_global_edges, HairFlow<DIM>::m_area_e, HairFlow<DIM>::m_parent); stringutils::print(HairFlow<DIM>::m_area_e);
    
    compute_vertex_area<DIM>(HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_area_e, HairFlow<DIM>::m_area_v_hair); stringutils::print(MAKE_REF(HairFlow<DIM>::m_area_v_hair));
    ...
    for(int i = 0; i < ne; ++i) HairFlow<DIM>::m_area_e(i) = area_e_global(HairFlow<DIM>::m_edge_indices[i]); stringutils::print(MAKE_REF(HairFlow<DIM>::m_area_e));
    
    for(int i = 0; i < np; ++i) HairFlow<DIM>::m_area_v(i) = area_v_global(HairFlow<DIM>::m_particle_indices[i]); stringutils::print(MAKE_REF(HairFlow<DIM>::m_area_v));
  ...
  compute_bracket_matrix(HairFlow<DIM>::m_area_e, m_Gf, DIM); stringutils::print(MAKE_REF(m_Gf));
  compute_bracket_matrix(HairFlow<DIM>::m_area_v, m_Gv); stringutils::print(MAKE_REF(m_Gv));
  compute_inv_bracket_matrix(HairFlow<DIM>::m_area_v, m_iGv); stringutils::print(MAKE_REF(m_iGv));
  
  compute_bracket_matrix(HairFlow<DIM>::m_area_v_hair, m_Gv_hair); stringutils::print(MAKE_REF(m_Gv_hair));
  compute_flat_bracket_matrix(HairFlow<DIM>::m_area_v_hair, m_area_v_hair_flat); stringutils::print(MAKE_REF(m_area_v_hair_flat));
  compute_inv_bracket_matrix(HairFlow<DIM>::m_area_v_hair, m_iGv_hair); stringutils::print(MAKE_REF(m_iGv_hair));
  
  // compute local interpolation matrix
  compute_edge_to_vertex_matrix(HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_area_v, HairFlow<DIM>::m_area_e, m_W_fv); stringutils::print(MAKE_REF(m_W_fv));
  compute_edge_to_vertex_matrix(HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_area_v_hair, HairFlow<DIM>::m_area_e, m_W_fv_hair); stringutils::print(MAKE_REF(m_W_fv_hair));
  
  // compute dirs
  compute_edge_dir<DIM>(x, HairFlow<DIM>::m_global_edges, HairFlow<DIM>::m_dir_f, HairFlow<DIM>::m_parent); stringutils::print(MAKE_REF(HairFlow<DIM>::m_dir_f));
  expand_dir_f(HairFlow<DIM>::m_dir_f, m_dir_f_expand); stringutils::print(MAKE_REF(m_dir_f_expand));
  compute_vertex_val(HairFlow<DIM>::m_dir_f, m_W_fv_hair, HairFlow<DIM>::m_dir_v);
  compute_normalized_vector<DIM>(HairFlow<DIM>::m_dir_v); stringutils::print(MAKE_REF(HairFlow<DIM>::m_dir_v));
  
  compute_vertex_val(HairFlow<DIM>::m_edge_rad_vec, m_W_fv_hair, HairFlow<DIM>::m_rad_vec); stringutils::print(MAKE_REF(HairFlow<DIM>::m_rad_vec));
    // compute local gradient matrix
  compute_edge_gradient_matrix<DIM>(x, HairFlow<DIM>::m_global_edges, HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_area_e, m_gradF, HairFlow<DIM>::m_parent); stringutils::print(MAKE_REF(m_gradF));
  
  // compute local divergence matrix
  compute_vertex_divergence_matrix(m_iGv, m_Gf, m_gradF, m_divV); stringutils::print(MAKE_REF(m_divV));
  
  // compute local divergence matrix
  compute_vertex_divergence_matrix(m_iGv_hair, m_Gf, m_gradF, m_divV_hair); stringutils::print(MAKE_REF(m_divV_hair));
  
  // compute local laplacian matrix
  compute_vertex_laplacian_matrix(m_gradF, m_divV_hair, m_L); stringutils::print(m_L);
  
  // compute local coordinates
  m_sum_area_e =
  compute_accumulated_edge_length(HairFlow<DIM>::m_area_e, m_area_e_accu); stringutils::print(MAKE_REF(m_area_e_accu));
  compute_inverted_pos_mapping(m_area_e_accu, m_sum_area_e, m_area_e_inv_mapping); stringutils::print(MAKE_REF(m_area_e_inv_mapping));
  compute_edge_val(m_area_e_accu, HairFlow<DIM>::m_internal_edges, m_edge_x); stringutils::print(MAKE_REF(m_edge_x));
  ...
  compute_edge_val(HairFlow<DIM>::m_porosity, HairFlow<DIM>::m_internal_edges, m_porosity_e); stringutils::print(MAKE_REF(m_porosity_e));
...
void CylindricalShallowFlow<DIM>::advance(const VectorXs& x, const scalar& dt)
{
  ...
  // advection
  compute_vertex_val(HairFlow<DIM>::m_u, m_W_fv_hair, m_uvert); stringutils::print(MAKE_REF(m_uvert));
  //std::cout << "[uvert]\n" << m_uvert << std::endl;
  
  compute_backtracing<DIM>(m_edge_x, dt, m_sum_area_e, m_area_e_accu, m_area_e_inv_mapping,
                           m_uvert, HairFlow<DIM>::m_c_v, m_ustar, HairFlow<DIM>::m_c_star); stringutils::print(MAKE_REF(m_ustar));
...
void CylindricalShallowFlow<DIM>::add_force(const VectorXs& x, const VectorXs& accel, FluidSim* fluidsim, const scalar& dt)
{
  ...
  if(HairFlow<DIM>::m_parent->isIndividualTransfer())
  {
    compute_next_pressure(m_L, HairFlow<DIM>::m_eta, HairFlow<DIM>::m_parent->getLiquidTension(), m_pressure); stringutils::print(m_pressure);
  } else {
  ...
  stringutils::print(MAKE_REF(m_pressure));

  auto& gv = HairFlow<DIM>::m_parent->getSimpleGravity();
  VectorXs g(DIM);
  for(int i = 0; i < DIM; ++i) g(i) = gv(i);
  // add force
  compute_accel_v(accel, HairFlow<DIM>::m_particle_indices, g, HairFlow<DIM>::m_accel_v, HairFlow<DIM>::m_parent); stringutils::print(MAKE_REF(HairFlow<DIM>::m_accel_v));
  
  
  compute_edge_val(HairFlow<DIM>::m_accel_v, HairFlow<DIM>::m_internal_edges, m_accel_f); stringutils::print(MAKE_REF(m_accel_f));
  
  compute_accel_e(m_accel_f, HairFlow<DIM>::m_dir_f, m_accel_e); stringutils::print(MAKE_REF(m_accel_e));
  ...
  compute_next_velocity(m_ustar,
                        ...
                        HairFlow<DIM>::m_u); stringutils::print(MAKE_REF(HairFlow<DIM>::m_u));
  ...
  compute_actual_u(HairFlow<DIM>::m_u, HairFlow<DIM>::m_dir_f, HairFlow<DIM>::m_actual_u_f);  stringutils::print(MAKE_REF(HairFlow<DIM>::m_actual_u_f));
  
  compute_vertex_val(HairFlow<DIM>::m_actual_u_f, m_W_fv_hair, HairFlow<DIM>::m_actual_u_v); stringutils::print(MAKE_REF(HairFlow<DIM>::m_actual_u_v));
}
...
void CylindricalShallowFlow<DIM>::preUpdateHairFlowHeight(const scalar& dt)
{
  ...
  compute_D_matrix(m_W_fv, HairFlow<DIM>::m_u, m_gradF, m_dir_f_expand, m_D); stringutils::print(MAKE_REF(m_D));
  compute_A_matrix(m_D, m_divV, HairFlow<DIM>::m_u, m_dir_f_expand, m_L, HairFlow<DIM>::m_parent->getHeightSmooth(), m_A); stringutils::print(MAKE_REF(m_A));
  
  compute_LHS_matrix(m_sp0, m_sp1, m_A, dt, m_LHS); stringutils::print(MAKE_REF(m_LHS));
...
{
  compute_edge_val(HairFlow<DIM>::m_eta, HairFlow<DIM>::m_internal_edges, HairFlow<DIM>::m_edge_eta); stringutils::print(MAKE_REF(HairFlow<DIM>::m_edge_eta));
...
void CylindricalShallowFlow<DIM>::updateHairFlowHeight(const scalar& dt)
{
  ...
  stringutils::print(MAKE_REF(m_rhs_plus));
  ...
  if(m_solver.info() == Eigen::Success)
  {
    ...
    stringutils::print(MAKE_REF(HairFlow<DIM>::m_eta));
    //std::cout << "[eta]\n" << HairFlow<DIM>::m_eta << std::endl;
