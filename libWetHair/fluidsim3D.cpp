...
void FluidSim3D::save_particles_off(const std::string szfn)
{
  using namespace std;
  ...
  {
    ofs << p.x(0) << " " << p.x(1) << " " << p.x(2) << " " << p.v(0) << " " << p.v(1) << " " << p.v(2) << " " << p.radii << endl;
  }
  ofs.close();
}

void FluidSim3D::load_particles_off(const std::string szfn)
{
  particles.clear();
  ...
  while(getline(ifs, line))
  {
    ...
    if(buf.size() >= 3) stringutils::extractFromString(buf[2], p.x(2));
    
    if(buf.size() >= 4) stringutils::extractFromString(buf[3], p.v(0));
    
    if(buf.size() >= 5) stringutils::extractFromString(buf[4], p.v(1));
    
    if(buf.size() >= 6) stringutils::extractFromString(buf[5], p.v(2));
