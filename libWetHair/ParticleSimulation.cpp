...
{
  assert( m_scene != NULL );
  m_scene->getSerializer().serializeScene( *m_scene, m_scene_stepper, outputstream );
}
...
{
  m_scene->getSerializer().serializeFluidReadable( *m_scene, osfluid );
  m_scene->getSerializer().serializeHairReadable( *m_scene, oshair );
  m_scene->getSerializer().serializeShallowFlowReadable( m_scene_renderer, *m_scene, osflow );
  m_scene->getSerializer().serializeBoundariesReadable( m_scene_renderer, *m_scene, os_boundary_single, os_boundary_double );
#ifdef WRITE_PC_VARS
  m_scene->getSerializer().serializePolygonalCohesionReadable(m_scene_renderer, *m_scene, os_pe, os_poe, os_ppp);
#endif
}
...
{
  bool f = m_scene->getSerializer().deSerializeFluidReadable( *m_scene, filename_fluids );
  bool h = m_scene->getSerializer().deSerializeHairReadable( *m_scene, filename_hairs );
  bool r = m_scene->getSerializer().deSerializeShallowFlowReadable( m_scene_renderer, *m_scene, filename_flows );
...
{
  assert( m_scene != NULL );
  m_scene->getSerializer().loadScene( *m_scene, m_scene_stepper, inputstream );
}
/////////////////////////////////////////////////////////////////////////////
// Status Functions
